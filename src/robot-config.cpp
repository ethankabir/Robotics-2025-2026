...
Chassis chassis(
    // Drivetrain motors - Left side (both reversed)
    mik::motor_group({
		mik::motor(PORT1, true, blue_6_1, "left_front_motor"),
		mik::motor(PORT11, true, blue_6_1, "left_back_motor")
    }),
    // Drivetrain motors - Right side (normal polarity)
    mik::motor_group({
		mik::motor(PORT20, false, blue_6_1, "right_front_motor"),
		mik::motor(PORT10, false, blue_6_1, "right_back_motor")
    }),

    PORT18, // Inertia sensor port - TODO: Update when you add an inertial sensor
    360,    // Inertial scale, value that reads after turning robot a full 360

    PORT19, // Forward Tracker Port - TODO: Update when you add tracking wheels
    ...
    PORT15,  // Sideways tracker port - TODO: Update when you add tracking wheels
    ...
    mik::distance_reset({
		// Distance sensor for wall resetting - TODO: Update when you add distance sensors
...
Assembly assembly(
	// Dual intake motors
	mik::motor_group({
		mik::motor(PORT12, false, blue_6_1, "intake_motor_1"),
		mik::motor(PORT2, false, blue_6_1, "intake_motor_2")
	}),
	// Pneumatic piston
	mik::piston(PORT_A)
);
...
void log_motors() {
    config_add_motors({
		...
		chassis.left_drive,
		chassis.right_drive,
		assembly.intake_motors
    },
	{
		// Add all individual mik motors in here (none for this robot)
    }
  );
}
